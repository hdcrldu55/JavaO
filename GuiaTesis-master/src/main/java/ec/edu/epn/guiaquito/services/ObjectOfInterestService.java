...
public class ObjectOfInterestService {
	...
	@PostConstruct
	private void init() {
		foursquareApi = new FoursquareApi("0OC52I2T5V3DBUGLL5LMXCCOEH1YAJT3BRDSYNVQGNXKTCVM", "V0CP0FXJARG52XADIQT34ITJPW1DGSRTMFLCZA22LVM3XF3K", null);
		try {
	...
	private List<ObjectOfInterest> convertToObjectOfInterest(List<CompactVenue> places, InterestType interestType) {
		...
		for (CompactVenue place : places) {
			...
			//TODO: from wikipedia o foursquare
			String description = "";
			try {
				Result<CompleteVenue> completeVenueResult = foursquareApi.venue(place.getId());
				description = completeVenueResult.getResult().getDescription();
			} catch (FoursquareApiException e) {
				e.printStackTrace();
			}
			if ("".equals(description) || description == null) {
				description = getDescriptionFromWikipedia(place.getLocation().getLat(), place.getLocation().getLng());
			}
