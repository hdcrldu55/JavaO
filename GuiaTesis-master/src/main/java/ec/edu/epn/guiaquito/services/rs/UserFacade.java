...
public class UserFacade extends BaseFacade<User, String> {
	...
	@EJB
	private OntologyService createO;
	...
	public Response create(User user) {
		User created;
		try {
			User byFacebookId = userDAO.findByFacebookId(user.getFacebookId());
			

			if (byFacebookId != null) {
				return update(byFacebookId);
			}
			created = userDAO.create(user);
			createO.createUserOntolgy(created);
		} catch (Exception e) {
			e.printStackTrace();
